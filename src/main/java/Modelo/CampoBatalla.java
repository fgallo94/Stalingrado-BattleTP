    ...
    private synchronized int traerAtaque() {
        ...
        for (Soldado s : atacanteG.getLista()) {
            if (s.getEstado() != 0) {
                ...
                setChanged();
                notifyObservers("\n El soldado " + atacanteG.getEjercito() + " ataca con " + s.getAtaque() + ", su daño es igual a " + s.getiAtaque());
                setChanged();
                notifyObservers("\n |||||||||||\n");
            }
        }
    ...
    private synchronized int traerDefensa() {
        ...
        for (Soldado s : defensorG.getLista()) {
            if (s.getEstado() != 0) {
                ...
                setChanged();
                notifyObservers("\n El soldado " + defensorG.getEjercito() + " defiende con " + s.getIDefensa() + ", su defensa es igual a " + s.getDefensa());
                setChanged();
                notifyObservers("\n |||||||||||\n");
            }
        }
